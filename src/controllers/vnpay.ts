...
export default class VnpayController {
  ...
  static async createPaymentUrl(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      // Init models
      ...
      const orderData = await orderModel.findOne({ _id: data.orderId }).lean().exec();
      ...
      const jobData = await jobModel.findOne({ _id: orderData.job }).populate('room').lean().exec();
      ...
      // Get ip
      data['ipAddr'] = req.headers['x-forwarded-for'] || req.connection.remoteAddress || req.socket.remoteAddress || req.socket.remoteAddress;

      // Init vnpay service
      const vnpayService = new VnpayService();

      return HttpResponse.returnSuccessResponse(res, await vnpayService.getRedirectUrl(data));
    } catch (e) {
  ...
  static async callBack(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      const {
        room: roomModel,
        floor: floorModel,
        ...
        job: jobModel,
        user: userModel,
        order: orderModel } = global.mongoModel;
      ...
      let orderData = await orderModel.findOne({ _id: orderId }).lean().exec();
      ...
      if (rspCode === '00') {
        ...
        return res.status(200).json({ RspCode: rspCode.toString(), Message: 'Order Failed' });
      }
  ...
  static async ipn(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      const {
        room: roomModel,
        floor: floorModel,
      ...
      if (rspCode === '00') {
        ...
        if (orderData.type === 'monthly') {
          await jobModel.findOneAndUpdate({ _id: orderData.job }, {
            isCompleted: true,
        ...
        if (orderData.type === 'deposit') {
          ...
          if (jobData.room.status === 'available') {
            await roomModel.findOneAndUpdate({ _id: jobData.room._id }, { status: 'deposited', rentedBy: jobData.user }, { new: true }).exec();
            let floorData = await floorModel.findOne({ rooms: jobData.room._id }).populate('rooms').lean().exec();
            const roomGroup = lodash.groupBy(floorData.rooms, (room) => {
              return room.status;
            });

            await floorModel.findOneAndUpdate({ _id: floorData._id }, {
              availableRoom: roomGroup['available'] ? roomGroup['available'].length : 0,
              rentedRoom: roomGroup['rented'] ? roomGroup['rented'].length : 0,
              depositedRoom: roomGroup['deposited'] ? roomGroup['deposited'].length : 0
            }).exec();

            let motelRoomData = await motelRoomModel.findOne({ floors: floorData._id }).populate('floors').lean().exec();
            ...
            await motelRoomModel.findOneAndUpdate({ _id: motelRoomData._id }, updateData).exec();
