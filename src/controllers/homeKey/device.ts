...
export default class DeviceController {
  ...
  static async createDevice(req: Request, res: Response, next: NextFunction): Promise<any> {
    try {
      // Init models
      ...
      return HttpResponse.returnSuccessResponse(res, await DeviceController.getDeviceById(deviceData._id));
    } catch (e) {
      next(e);
    }
  }
  ...
  static async getDeviceById(deviceId: any, lang?: string): Promise<any> {
    ...
    let resData = await deviceModel.findOne({ _id: deviceId }).populate('children').lean().exec();
